...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # print(hidden_states.shape)  # [1, x, 4096] --> [1, 1, 4096],  x = seq_len
        # print(attention_mask.shape)  # [1, 1, x, x] --> [1, 1, 1, x+1]
        # print(position_ids.shape)  # [1, x] --> [1, 1]
        ...
        # print(query_states.shape)  # [1, x, 4096] --> [1, 1, 4096]
        # print(key_states.shape)  # [1, x, 4096] --> [1, 1, 4096]
        # print(value_states.shape)  # [1, x, 4096] --> [1, 1, 4096]
        ...
        kv_seq_len = key_states.shape[-2]  # x --> 1
        ...
        # print(type(past_key_value))  # <class 'transformers.cache_utils.DynamicCache'>
        ...
        # print(self.num_key_value_groups)  # 1
        # print(query_states.shape)  # [1, 32, x, 128] --> [1, 32, 1, 128]
        # print(key_states.shape)  # [1, 32, x, 128] --> [1, 32, x+1, 128]
        # print(value_states.shape)  # [1, 32, x, 128] --> [1, 32, x+1, 128]

        attn_weights = torch.matmul(query_states, key_states.transpose(2, 3)) / math.sqrt(self.head_dim)

        # print(attn_weights.shape)  # [1, 32, x, x]  --> [1, 32, 1, x+1]
        # print(attention_mask)  # [1, 32, x, x] upper right are -inf --> [1, 1, 1, x+1] all 0
        ...
        if attention_mask is not None:
            if attention_mask.size() != (bsz, 1, q_len, kv_seq_len):
                raise ValueError(
                    ...
                    # ValueError: Attention mask should be of size (1, 1, 1, 1), but is torch.Size([1, 1, 1, 285])
                )
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            # print('input_ids', input_ids.shape, attention_mask.shape)
            batch_size, seq_length = input_ids.shape[:2]
        elif inputs_embeds is not None:
            # print('inputs_embeds', inputs_embeds.shape, attention_mask.shape)
        ...
        if self._use_flash_attention_2:
            ...
            # print(attention_mask.shape, (batch_size, seq_length), past_key_values_length)
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        ...
        # print(type(past_key_values))  # <class 'NoneType'> --> <class 'tuple'>
        ...
        if past_key_values is not None:
            ...
            # print(isinstance(past_key_values, Cache))  # False
            # print(cache_length, past_length, max_cache_length)  # x, x, None
            # print(input_ids.shape)  # (1, x+1)
            # print(attention_mask.shape)  # (1, x+1)
...
class AugLlamaModel(LlamaModel):
    def init_aug(self, aug_models, num_cross=8):
        self.requires_grad_(False)
        # self.aug_models = nn.ModuleList(aug_models)
        self.aug_models = aug_models
        self.main_device = torch.device('cuda:0')
        self.backup_device = torch.device('cuda:1')
        for i in range(len(self.aug_models)):
            self.aug_models[i].requires_grad_(False)
            self.aug_models[i].to(self.backup_device)
        if num_cross == 8:
            ...
            # for i in range(len(self.aug_models)):
            #     self.cross_attentions.append(nn.ModuleList(
    ...
    def forward(
        ...
        aug_hidden_states = [aug_outputs[i].hidden_states[1:] for i in range(len(self.aug_models))]
        ...
        if self._use_flash_attention_2:
            ...
            # 4d mask is passed through the layers
            # print((batch_size, seq_length), inputs_embeds, past_key_values_length)
            attention_mask = _prepare_4d_causal_attention_mask(
                attention_mask, (batch_size, seq_length), inputs_embeds, past_key_values_length
            )
        ...
        for decoder_layer_id, decoder_layer in enumerate(self.layers):
            ...
            if decoder_layer_id in self.attention_layer_idx:
                ...
                attn_outputs = []
                for i in range(len(self.aug_models)):
                    encoder_hidden_states = aug_hidden_states[i][decoder_layer_id].to(self.main_device)
                    attn_output, _ = self.cross_attentions[i][cross_attention_id](
                        hidden_states=hidden_states,
                        attention_mask=attention_mask,
                ...
                for i in range(len(self.aug_models)):
                    attn_weight = 1e-3 if i == 1 else 1e-3
...
class AugLlamaForCausalLM(LlamaForCausalLM):
    ...
    def init_aug(self, aug_models, num_cross=8):
        self.requires_grad_(False)
        self.model.init_aug(aug_models, num_cross)
    ...
    def load(self, paths):
        for i, path in enumerate(paths):
            ...
            modified_state_dict = {}
            for key, value in cross_attention_dict.items():
                new_key = key.replace('model.cross_attentions', f'model.cross_attentions.{i}')
